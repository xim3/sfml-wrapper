...
, lastY(0)
{
    ...
    bgTiles.setPrimitiveType(sf::Quads);
    bgTiles.resize(width_in_tiles * height_in_tiles * 4);
}
void ItemGrid::loadTex(sf::Texture* tex){
	_tex = tex;
}
void ItemGrid::resizeBg()
{
    bgTiles.setPrimitiveType(sf::Quads);
    bgTiles.resize(width_in_tiles * height_in_tiles * 4);
}
void ItemGrid::setBgVisible(bool v)
{
    has_backgrd = v;
}
bool ItemGrid::getBgVisible() const
{
    return has_backgrd;
}
void ItemGrid::appendItem(const int _x, const int _y, const unsigned int GID, bool layer){
    lastX = _x;
    lastY = _y;
	...
	if(layer == 1)
        quad = &bgTiles[(lastX + lastY * width_in_tiles) * 4];
